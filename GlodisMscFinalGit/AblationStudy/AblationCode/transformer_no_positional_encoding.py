...
Ablation to assess the contribution of positional encodings. Trains a Transformer variant
without explicit positional encodings and reports the impact on error metrics.
...
Reproducibility
---------------
- Seed numpy / tensorflow.
- Ensure consistent preprocessing (scalers) between ablation and baseline.

Example
-------
python transformer_no_positional_encoding.py --models-dir Models --epochs 50
.
"""
...
def transformer_block(x, d_model=128, ff_dim=256, num_heads=4, dropout=0.1, causal=True):
...
def build_model(seq_len: int, in_dim: int = 3, d_model: int = 128, ff_dim: int = 256, num_heads: int = 4):
    inputs = keras.Input(shape=(seq_len, in_dim))      
    x = layers.Dense(d_model)(inputs)                 
    # NOTE: PositionalEncoding intentionally REMOVED in this ablation
...
def main():
    ...
    model = build_model(seq_len=args.seq_len, in_dim=in_dim, d_model=128, ff_dim=256, num_heads=4)
    ...
    print(f"[No-PE] Test MSE: {mse:.6f} | Curvature: {curv:.6f}")

    
    model_path = os.path.join(args.models_dir, "transformer_no_posenc.keras")
    xsc_path  = os.path.join(args.models_dir, "transformer_no_posenc_x_scaler.pkl")
    ysc_path  = os.path.join(args.models_dir, "transformer_no_posenc_y_scaler.pkl")
    model.save(model_path)
    joblib.dump(scaler_x, xsc_path)
    joblib.dump(scaler_y, ysc_path)

    metrics = {
        "ablation": "positional_encoding_removed",
    ...
    with open(os.path.join(args.results_dir, "ablation_posenc_metrics.json"), "w") as f:
        json.dump(metrics, f, indent=2)
    print(f"[save] {model_path}\n[save] {xsc_path}\n[save] {ysc_path}\n[save] results/ablation_posenc_metrics.json")
